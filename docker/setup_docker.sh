...
if [ "$1" == "client" ]; then
	...
	docker run -v $PWD/client:/app/tabledb/setup/server/ --network mynet --ip 172.16.0.4 -d tabledb client 
else 
	for i in 11 12 13
	do
	echo start server$i
	...
	docker run -v $PWD/server$i:/app/tabledb/setup/server/ --network mynet --ip 172.16.0.$i -d tabledb server $i
